...
- Message retrieval (`GET /messages` and `GET /user/messages`) supports pagination via query parameters: `limit` maximum items in result page, `next_result` to get the next result page (`next_result` value is provided in the server's response along with the messages if more messages can be retrived; Otherwise it is empty).
